...
export default function AdminCandidaturas() {
  ...
  async function markRead(id) {
    await supabase.from('applications').update({ leido: true }).eq('id', id)
    setRows(prev => prev.map(r => r.id === id ? { ...r, leido: true } : r))
    if (selected?.id === id) setSelected(prev => ({ ...prev, leido: true }))
    setCounts(prev => ({ ...prev, app: Math.max(0, prev.app - 1) }))
  }
  ...
  async function handleSelect(row) {
    ...
    if (row.cv_url) {
      const { data } = await supabase.storage
        ...
        .createSignedUrl(row.cv_url, 60 * 60 * 24)
    ...
    <>
      ...
      <div className={styles.content}>
        ...
        {loading ? (
          ...
          <div className={styles.empty}><p>No hay candidaturas todavía.</p></div>
        ) : (
          <div className={styles.splitWrap}>
                ...
                <tbody>
                  {rows.map(r => (
                    ...
                    >
                      ...
                      <td><div className={styles.cellSub}>{r.puesto_interes || '—'}</div></td>
            ...
            <div className={styles.detailPanel}>
              {!selected ? (
                <p className={styles.detailPanelEmpty}>Haz clic en una candidatura para verla</p>
              ) : (
                <>
                  ...
                  <div className={styles.detailField}>
                    <div className={styles.detailLabel}>Puesto de interés</div>
                    <div className={styles.detailValue}>{selected.puesto_interes}</div>
                  </div>
                  {selected.mensaje && (
                    <div className={styles.detailField}>
                      <div className={styles.detailLabel}>Carta de presentación</div>
                  ...
                  <div style={{ marginTop: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <a
                      href={`mailto:${selected.email}?subject=Tu candidatura en FRICAL`}
                    ...
                    {selected.cv_url && !cvUrl && (
                      <span className={`${styles.btn} ${styles.btnSecondary} ${styles.btnSm}`} style={{ opacity: .5 }}>
                        Generando enlace CV…
                      </span>
                    )}
                  </div>
